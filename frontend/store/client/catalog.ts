...
export const useCatalog = defineStore("catalogStore", () => {
  ...
  function sortCatalog(a: Client.GoodPreview, b: Client.GoodPreview) {
    ...
    if (selectedSort.value === "price-down") {
      return b.price! - a.price!;
    }
    if (selectedSort.value === "price-up") {
      ...
      currentCatalog.value == null
        ...
        : {
            ...
            goods: currentCatalog.value.goods
              .filter(
                (good) =>
                  good.price! >= selectedFilter.value.from &&
                  good.price! <= selectedFilter.value.to
              )
              .sort(sortCatalog),
          };
  }
