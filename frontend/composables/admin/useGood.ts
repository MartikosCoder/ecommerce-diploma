...
export function useGood() {
  ...
  watch(
    () => goodsStore.currentGood,
    () => {
      if (goodsStore.currentGood == null) {
        return;
      }
  ...
  async function save(id: number) {
    ...
    const sendData = {
      ...
      id,
      category_id:
        data.value.category_id == null ? null : +data.value.category_id,
    ...
    const result =
      id === 0
        ? await goodsStore.addGood(sendData)
        : await goodsStore.updateGood(sendData);
    pending.value = false;
