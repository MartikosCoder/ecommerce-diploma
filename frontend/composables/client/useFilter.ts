...
export function useFilter() {
  ...
  function applyFilter() {
    if (filterValue.value.from != null && filterValue.value.to != null) {
      ...
      if (fromValue < 0) {
        filterValue.value.from = "0.00";
      }
      if (fromValue >= 1000000) {
        filterValue.value.from = "999999.99";
      }

      if (toValue < 0) {
        filterValue.value.to = "0.00";
      }
      if (toValue >= 1000000) {
        filterValue.value.to = "999999.99";
      }

      if (fromValue > toValue) {
        filterValue.value.to = filterValue.value.from;
      }
    }
    ...
    useEvent("catalog:update", {
      filter: {
        from: +(filterValue.value.from || 0),
        to: +(filterValue.value.to || 999999.99),
      },
    });
  }
