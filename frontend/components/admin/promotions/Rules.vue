...
<template>
  <section class="max-h-96 overflow-auto flex flex-col gap-5">
    <header class="flex gap-2 items-center text-xl">
      <h2>{{ $t("admin.promotions.rulesTitle") }}</h2>
    ...
    <article class="flex flex-col gap-5">
      <div
        v-for="rule in rules"
        :key="rule.id"
        class="flex items-center gap-5 pb-5 border-b last:border-b-0 border-slate-300"
      >
        <Icon
          class="text-3xl cursor-pointer text-red-500 hover:text-red-600 active:text-red-700 transition-colors"
