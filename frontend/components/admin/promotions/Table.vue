...
<template>
  <table
    class="mt-10 text-xl overflow-auto w-full border-collapse border border-slate-500"
  >
    ...
    <tbody v-if="promotionsStore.promotions.length > 0">
      <tr
        v-for="promotion in promotionsStore.promotions"
        :key="promotion.id"
      ...
      >
        ...
        <td class="text-center">
          <button
            ...
            @click="emit('openModal', promotion.id)"
          >
            {{ $t("admin.showDetails") }}
          </button>
        </td>
