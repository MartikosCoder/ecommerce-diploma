...
const cannotClose = computed(
  () => (appealsStore.currentAppeal?.status as string) !== "in-work"
);
...
async function askClose() {
  ...
  if (!(await appealsStore.closeAppeal())) {
    return;
  }
