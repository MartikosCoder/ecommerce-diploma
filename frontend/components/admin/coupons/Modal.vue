...
async function askDelete() {
  ...
  if (!(await couponsStore.deleteCoupon(props.id))) {
    return;
  }
    ...
    >
      ...
      <article class="flex flex-col gap-5">
        <div class="grid grid-cols-2 gap-2">
          ...
          />
          <AdminUiInput id="code" v-model="data.code" type="text" label="Код" />
          <AdminUiInput
            id="use_amount"
            v-model="data.use_amount"
            type="number"
            :label="$t('admin.coupons.modalUseAmount')"
          />
        </div>
