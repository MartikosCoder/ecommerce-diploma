...
const menuItems = computed(() => [
  ...
  },
  {
    ...
    title: user?.name ?? t("menu.profile"),
    link: "/profile",
  },
]);
...
async function navigate(id: number, link: string) {
  if (onlyAuthItems.includes(id) && !(await checkForAuth())) {
    return;
  }
...
<template>
  <nav
    class="w-full z-10 mt-auto pt-1 sticky bottom-0 bg-white flex sm:gap-5 justify-center transition-all md:gap-10"
  >
    <ClientUiMenuItem
