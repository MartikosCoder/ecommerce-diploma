...
<template>
  <div class="flex flex-col gap-3 items-center">
    <div>
      <img
        class="aspect-square object-contain max-h-20"
        :src="good.img"
        alt=""
        onerror="this.src = '/img/goods_placeholder.png'"
      />
    </div>
    <span class="overflow-hidden max-w-[4rem] max-h-8 text-lg text-ellipsis">{{
      good.title
    }}</span>
    <span class="font-bold">{{ good.price }} ₴</span>
    ...
    <div class="flex gap-2">
      ...
      <ClientUiIconButton
        ...
        @click="cartStore.toggleCartItem(good.id)"
      />
      <ClientUiIconButton
      ...
      <ClientUiIconButton
        ...
        @click="comparisonStore.toggleComparison(good.id)"
      />
    </div>
  </div>
</template>
