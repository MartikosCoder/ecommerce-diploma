...
<template>
  <article class="px-2 lg:px-10 w-full flex flex-col gap-5">
    <h1 class="text-2xl lg:text-3xl font-bold">
      {{ $t("client.profile.deliveries.menu") }}
    </h1>
    <div class="flex flex-col gap-2 justify-center">
      <div
        v-for="delivery in deliveriesStore.deliveries"
        :key="delivery.id"
        class="md:text-xl p-5 border-b border-blue-950 flex items-center justify-between"
      >
        {{ deliveryOption(delivery) }}
        <div class="w-1/4 flex gap-1 justify-center">
          <ClientUiIconButton
            name="material-symbols:edit-rounded"
            @click="toggleModal(delivery.id)"
          />
          <ClientUiIconButton
            name="material-symbols:delete-outline-rounded"
            @click="askDelete(delivery.id)"
          />
        </div>
      </div>
    </div>
    <button
      ...
      @click="toggleModal()"
    >
      {{ $t("client.profile.deliveries.add") }}
    </button>
