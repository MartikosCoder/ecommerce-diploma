    ...
    >
      <header class="flex justify-between items-center">
        <h2 class="text-lg font-bold md:text-2xl">
          {{ $t("client.profile.appeals.modalTitle") }}
        </h2>
        <ClientUiIconButton
        ...
        >
          <div class="flex gap-2 items-center">
            ...
            >
              {{
                message.from_admin
                  ? appealsStore.currentAppeal?.admin?.name
                  : $t("messageMe")
              }}
            </h3>
            <p class="text-sm">
      ...
      <footer
        class="mt-auto flex flex-col md:flex-row w-full justify-center gap-5"
      >
        <ClientUiInput
          v-model:value="data.text"
          :label="$t('client.profile.appeals.message')"
          type="text"
          :placeholder="$t('client.profile.appeals.problem')"
        />
        <button
          :disabled="cannotSave"
          class="bg-blue-700 text-white px-5 py-3 rounded-md self-end transition-colors disabled:bg-slate-400 hover:bg-blue-800 active:bg-blue-900"
          @click="save"
        >
          {{ $t("send") }}
        </button>
