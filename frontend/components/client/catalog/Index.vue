...
<template>
  <article
    v-if="catalogStore.formattedCatalog"
  ...
  >
    ...
    <ClientCatalogMenu />
    <section
      v-if="catalogStore.formattedCatalog.goods.length > 0"
      class="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4"
    >
      <ClientUiCatalogItem
        v-for="good in catalogStore.formattedCatalog.goods"
        :key="good.id"
        :good="good"
    ...
    <section v-else class="m-auto">
      <h2 class="text-xl lg:text-2xl">{{ $t("client.catalog.noGoods") }}</h2>
    </section>
  </article>
</template>
