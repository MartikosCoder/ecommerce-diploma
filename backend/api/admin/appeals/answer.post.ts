...
export default defineEventHandler(async (event) => {
  ...
  const props = ["id", "message"] as Array<
    keyof { id: number; message: string }
  >;
  ...
  await pool.query(
    "UPDATE \"Appeals\" SET admin_id = $2, status = 'in-work' WHERE id = $1 AND status = 'new'",
    [body.id, tokenInfo!.id]
  );

  const appealSQL = await pool.query(
    'SELECT id FROM "Appeals" WHERE id = $1 AND admin_id = $2',
    [body.id, tokenInfo!.id]
  );
  ...
  await pool.query(
    "INSERT INTO \"Appeal_Messages\"(appeal_id, from_admin, message) VALUES ($1, 'true', $2)",
