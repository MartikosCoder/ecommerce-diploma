...
export default defineEventHandler(async (event) => {
  ...
  if (appealId === 0) {
    const appealSQL = await pool.query(
      'INSERT INTO "Appeals"(user_id) VALUES ($1) RETURNING id',
      [tokenInfo!.id]
    );
  ...
  await pool.query(
    "INSERT INTO \"Appeal_Messages\"(appeal_id, from_admin, message) VALUES ($1, 'false', $2)",
    [appealId, body.text]
  );
  ...
  const messagesSQL = await pool.query(
    'SELECT from_admin, sent_time, message FROM "Appeal_Messages" WHERE appeal_id = $1',
    [appeal.id]
  );
